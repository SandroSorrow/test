    ...
    if form_reg.validate_on_submit():
        ...
        if key == reg_key:
            if password1 == password2:
                password = password1
                id_ = functions.registration(email, password, file)
                return redirect(url_for('new_account', login=id_))
            else:
                return render_template("notifications.html",
            ...
            return render_template("notifications.html",
                                   notification="Please, input registration key.",
                                   back='registration',)
...
def account(login):
    ...
    return render_template("account.html", login=login, rank=rank,
                           name=name,
                           sex=sex,
                           surname=surname,
                           birthday=birthday,
...
@app.route('/account_id<login>/missions/stat_<status>/report<inc_id>', methods=['get', 'post'])
